...
Use `mm reset` only after checking version, auth status, current environment, and the failing command's help output. Reset clears local session state and should not be the first troubleshooting step. Always ask the user for explicit confirmation before running reset.
