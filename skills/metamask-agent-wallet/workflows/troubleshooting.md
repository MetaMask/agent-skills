...
Use `mm-dev reset` only after checking version, auth status, current environment, and the failing command's help output. Reset clears local session state and should not be the first troubleshooting step.
