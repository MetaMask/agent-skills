...
fallback; the script also checks `_meta["x402/error"]` (see
[Cloudflare Agents SDK servers](#cloudflare-agents-sdk-servers-agentsx402) in Notes). Only x402
...
For an HTTP `402`, run `inspect` and show the user the asset, amount, network, `payTo`, and
resource; run `pay ... --confirm` only after the user approves. For a paid MCP tool call, run
`mcp-inspect` and `mcp-sign ... --confirm` instead — same fields, same approval gate, just the
MCP-specific commands, not a fallback for when `inspect`/`pay` fail. A signature authorizes a real
token debit — over MCP that holds even though the script never touches the network, because the
signed payload settles when the caller replays the tool call.
